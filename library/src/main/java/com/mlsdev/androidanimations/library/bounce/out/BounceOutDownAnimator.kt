...
class BounceOutDownAnimator(view: View, animationParams: AnimationParams = AnimationParams()) : BaseViewAnimator(view, animationParams)  {
    override fun prepare(target: View, animatorSet: AnimatorSet) {
        val targetHeight = target.height.toFloat()
        animatorSet.playTogether(
            ObjectAnimator.ofFloat(target, "alpha", 1.0f, 1.0f, 1.0f, 0.0f),
            ObjectAnimator.ofFloat(target, "translationY", 0.0f, 20.0f, -10.0f, targetHeight)
        )
    }
}